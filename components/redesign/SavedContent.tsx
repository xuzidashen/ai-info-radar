  ...
  return (
    <>
      ...
      <section className="app-card mt-5 overflow-hidden">
        ...
        {((showArticles && !articles.length && !showInsights) || (showInsights && !insights.length && !showArticles) || (showArticles && !articles.length && showInsights && !insights.length)) ? (
          <div className="p-10 text-center">
            <BookmarkSimple size={30} className="mx-auto text-[var(--app-text-muted)]" />
            <p className="mt-3 font-black">这里还没有收藏</p>
            <p className="mt-2 text-sm font-semibold text-[var(--app-text-muted)]">运行主题更新后，可以在分析结果页收藏值得保留的内容。</p>
            <div className="mt-5 flex flex-wrap justify-center gap-2">
              <Link href="/topics" className="app-button">我的关注</Link>
