...
function usesContentExperience(pathname: string) {
  return pathname === "/" || productRoutes.some((route) => pathname === route || pathname.startsWith(`${route}/`));
}

export function AppShell({ children }: { children: ReactNode }) {
  const pathname = usePathname();

  if (usesContentExperience(pathname)) {
    return children;
  }
